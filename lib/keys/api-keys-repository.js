 ...
 * @property {string|null} openrouter_delete_hash  OpenRouter delete handle (for revoke), NOT a hash of the key.
...
export async function activate(id, { hash, hint, rawKey }) {
  ...
  await sql`
    ...
    set openrouter_delete_hash = ${hash}, openrouter_key = ${rawKey}, key_hint = ${hint}, status = 'active'
    where id = ${id}`;
}

 ...
 * API ref (`limit_reset`, `include_byok_in_limit`, `workspace_id`).
 *
 ...
 */
export async function createKey({ name, limitUsd, resetPeriod, includeByok, expiresAt, workspaceId }) {
  const res = await fetch(OPENROUTER_KEYS_URL, {
    ...
    body: JSON.stringify(buildCreateKeyBody({ name, limitUsd, resetPeriod, includeByok, expiresAt, workspaceId })),
  });
